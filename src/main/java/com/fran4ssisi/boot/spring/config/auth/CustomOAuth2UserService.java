...
import com.fran4ssisi.boot.spring.config.auth.dto.OAuthAttributes;
import com.fran4ssisi.boot.spring.config.auth.dto.SessionUser;
import com.fran4ssisi.boot.spring.domain.user.User;
import com.fran4ssisi.boot.spring.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        * 지금은 구글만 사용한 불필요한 값이지만, 이후 네이버 로그인 연동 시에 네이버 로그인인지, 구글 로그인인지 구분하기 위해 사용
        * */
        String registrationId = userRequest.getClientRegistration().getRegistrationId();
        /*userNameAttributeName
        * OAuth2 로그인 진행 시 키가 되는 필드값을 이야기한다. Primary Key와 같은 의미
        * 구글의 경우 기본적으로 코드를 지원하나, 네이버 카카오등은 기본 지원하지 않는다. 구글의 기본코드는 "sub"이다.
        ...
        * */
        ...
        httpSession.setAttribute("user", new SessionUser(user));//SessionUser 세션에 사용자 정보를 저장하기위한 Dto클래스
